...
def generar():
    try:
        ...
        # ========= IMAGEN ORIGINAL (NO SE TOCA) =========
        carnet = Image.open(plantilla_path).convert("RGBA")
        draw = ImageDraw.Draw(carnet)

        # ========= SOLO CAMBIO REAL =========
        try:
            font = ImageFont.truetype("arial.ttf", 120)  # ⬅️ ANTES 48
        except:
        ...
        draw.text((x, y + 160), f"Documento: {documento}", font=font, fill=color)
        draw.text((x, y + 320), f"Cargo: {cargo}", font=font, fill=color)
